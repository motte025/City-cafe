...
import type {BallPlan,FreeThrowRequest} from './ball-plan';
...
export class Wheel {
 ...
 private planRequest(index:number,direction:1|-1,rotorStart:number,rotorSpeed:number):FreeThrowRequest{
  return {shape:{...this.shape},ball:{diameter:this.ballDiameter,mass:this.ballMass,bounce:this.ballBounce},direction,rotorStart,rotorSpeed,
   launchAngle:launchPosition(rotorStart,this.lastIndex),
 ...
 private current(request:FreeThrowRequest){const b=request.ball,dr=request.deflectorResistance;return sameShape(request.shape,this.shape)&&b.diameter===this.ballDiameter&&b.mass===this.ballMass&&b.bounce===this.ballBounce&&dr?.radial===this.deflectorResistanceRadial&&dr?.tangential===this.deflectorResistanceTangential;}
 ...
 private beginSpin(p:NonNullable<Wheel['preparation']>){
  const {index:fallbackIndex,variant,direction}=p,plan=this.readyPlan(p.ticket),initialBall=launchPosition(this.angle,this.lastIndex);
  this.stats.throws++;
  if(plan){
   // Freier Wurf: die Zahl ist plan.finalIndex – wohin die Physik tatsächlich gerollt ist.
   // fallbackIndex war vorab nur für den seltenen Notfall gezogen und wird sonst nie verwendet.
   const index=plan.finalIndex;
