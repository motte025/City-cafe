 ...
 */
import type {PlanRequest,PlanResult} from './ball-plan';

export interface PlanTicket {id:number;request:PlanRequest;result:PlanResult|null;started:number;finished:number}
...
export class BallPlanner {
 ...
 request(request:PlanRequest):PlanTicket{
 ...
 private async runLocal(ticket:PlanTicket){
  const {planThrow}=await import('./ball-plan');
  const search=planThrow(ticket.request);
  const slice=()=>{
