...
import {planThrow,type PlanRequest} from './ball-plan';
let current=0;
self.onmessage=(event:MessageEvent<{id:number;request:PlanRequest}>)=>{current=event.data.id;void run(event.data.id,event.data.request);};
async function run(id:number,request:PlanRequest){
 const search=planThrow(request);let step=search.next();
 while(!step.done){
